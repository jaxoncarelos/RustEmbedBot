...
pub fn get_regex(content: &Content) -> String {
    let regex_map: HashMap<Content, String> = HashMap::from([
        (Content::Twitter, r"https:\/\/(?:www\.)?(twitter|x)\.com\/.+\/status(?:es)?\/(\d+)(?:.+ )?".to_string()),
        (Content::Tiktok, r"https?://(?:www\.|vm\.|vt\.)?tiktok\.com/.+(?: )?".to_string()),
        (Content::Reddit, r"https?://(?:(?:old\.|www\.)?reddit\.com|v\.redd\.it)/.+(?: )?".to_string()),
        (Content::Instagram, r"https?:\/\/(?:www\.)?instagram\.com\/[a-zA-Z0-9_]+\/?(?:\?igshid=[a-zA-Z0-9_]+)?".to_string()),
        (Content::Facebook, r"https?:\/\/(?:www\.)?facebook\.com\/(reel)\/[a-zA-Z0-9_]+\/?".to_string())
    ]);
    let winner = regex_map.iter().find(|(key, _)| {
        key == &content
    });
    match winner {
